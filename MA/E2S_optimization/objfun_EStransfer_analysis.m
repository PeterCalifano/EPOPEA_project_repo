function [DV, data] = objfun_EStransfer_analysis(nlpvar, planets_id, Ra_target, Rp_target)
%% PROTOTYPE
% [DV, data] = objfun_EStransfer_analysis(nlpvar, planets_id, Ra_target, Rp_target)
% -------------------------------------------------------------------------------------------------------------
%% DESCRIPTION
% Ojective function for the Earth-Saturn interplanetary transfer
% -------------------------------------------------------------------------------------------------------------
%% INPUT
% nlpvar - NLP decision variables vector
...
% data: [struct] with fields: vinf_minusplus, DVs, Xplanets, EventEpochsJD
% -------------------------------------------------------------------------------------------------------------
%% CHANGELOG
% 17/11/2022 - Matteo Lusvarghi - First version
% 30/12/2022 - Pietro Califano - Adapted for flyby analysis
% -------------------------------------------------------------------------------------------------------------
...
for i = 1:N+1
    ...
    if i ~= (N+1)
        ...
        vinf_minusplus(i, :) = [v_inf_in', v_inf_out'];
...
data.vinf_minusplus = vinf_minusplus.*VU;
