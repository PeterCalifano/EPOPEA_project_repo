...
% P_cold_land = 389; Electrical power produced by RTG
P_cold_land = 327.12-20+9.43-50; % science mode power budget - power TTMTC + dissipated by TTMTC - POWER DEDICATED TO TCS (TO SIZE)
...
C.C_15 = k_TS * A_1/L_TS; % conductive coupling between the internal node and the radiator
R.R_50 = A_1 * eps_rad;
%%% Heat Flows
...
Q_ext = [0; % or q_Sat * A_1*eps_rad*(A_1*epsilon_1*eps_rad)/(A_1*epsilon_1*eps_rad+A_1*eps_rad)
        q_Enc * A_2*epsilon_ext*cos(theta_2Sat);
        q_Sat * A_3*epsilon_ext*cos(theta_3Sat);
        q_Sat * A_4*epsilon_ext*cos(theta_4Enc);
        q_Sat * A_1*eps_rad*cos(theta_4Enc) ];
...
% Cold case
T_land_cold = fsolve(@(T) HeatBalance_Lander_new(T, R,C, Q_ext , Q_diss, sigma_SB),T_guess, options);
