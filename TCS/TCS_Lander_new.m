...
eff_shunt = 0.6;
P_tot_cold_land = 389; %Electrical power produced by RTG
P_cold_land = P_tot_cold_land*eff_shunt -20 + 9.43 * 2 ; % science mode power budget - power TTMTC + dissipated by TTMTC - POWER DEDICATED TO TCS (TO SIZE)
% P_cold_land = 186; % during SK
% Power budget cold = 327.12;
...
%%% Radiator
% (since the radiators exchange heat directly with outside they're limike 
% a direct link between 1 and T0 = 0 K)

% R.R_10 = A_5 * eps_rad;
% R.R_10 = 1/(1/(A_5*eps_rad)+1/(A_5*epsilon_1*eps_rad)); % ???????????
C.C_15 = k_TS * A_5/L_TS; % conductive coupling between the internal node and the radiator
R.R_50 = A_5 * eps_rad;
...
Q_ext_cold = [0; % or q_Sat * A_5*eps_rad*(A_5*epsilon_1*eps_rad)/(A_5*epsilon_1*eps_rad+A_5*eps_rad)
        q_Enc * A_2*epsilon_ext*cos(theta_2Enc);
        q_Sat * A_3*epsilon_ext*cos(theta_3Sat);
        q_Sat * A_4*epsilon_ext*cos(theta_4Sat);
        q_Sat * A_5*eps_rad*cos(theta_4Sat)];

%%% Internal dissipation power
Q_diss_cold =  P_cold_land + n_RHU * P_RHU;

%%% SOLVE THE SYSTEM 
T_guess = 273*ones(5,1);
...
% Adjustments in data 
eps_rad_open = 0.7;
A_4h = (3*L^2 - n_rad*A_rad);
R.R_50 = A_5 * eps_rad_open;

% Electrical Power to dissipate internally
eff_shunt = 0.5;
P_hot_land = 456*eff_shunt; 
...
theta_3Sun = 10 * pi/180;
theta_4Sun = 80 * pi/180;
theta_5Sun = theta_4Sun;

% Overall external heat to each node (dissipation is not included)
Q_ext_hot = [0;
        q_Earth*epsilon_ext*A_2 + q_alb*A_2*alpha_ext ;
        q_Sun * A_3 * alpha_ext * cos(theta_3Sun) ;
        q_Sun * (A_4h - L^2) * alpha_ext * cos(theta_4Sun) ;
        q_Sun * A_5 * alpha_louv_open * cos(theta_5Sun)];

%%% Internal dissipation power
Q_diss_hot =  P_hot_land + n_RHU * P_RHU;
...
fprintf(['\nExternal Lateral: ',num2str(T_land_hot(4)-273),' Celsius\n'])
fprintf(['\nInternal T: ',num2str(T_land_hot(1)-273),' Celsius\n'])