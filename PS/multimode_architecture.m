...
V_fu_main = (m_fu_sk + m_fu_main) / rho_f ; % [m3] - Volume of the fuel
V_ox_main = (m_ox_sk + m_ox_main) / rho_ox ; % [m3] - Volume of the oxidizer (imposed to be equal)
...
%% PRESSURE REGULATED - BIPROPELLANT THRUSTER (MAIN+SK)
...
V_prop_sec = V_prop_att ;

%% CUMULATIVE MASSES
m_gas = m_gas_main + m_gas_att ; % [kg] - Overall pressurant mass
m_ox = m_ox_main +m_ox_sk ; % [kg] - Overall oxidizer mass
m_fu = m_fu_main + m_prop_att + m_fu_sk ; % [kg] - Overall fuel mass

V_gas = V_gas_main + V_gas_att ; % [m3] - Overall pressurant mass 
V_ox = ( V_ox_main ) * 1.1 ; % [m3] - Overall oxidizer volume + Margin MAR-CP-010

V_fu = ( V_fu_main + V_prop_att ) * 1.1 ; % [m3] - Overall fuel volume + Margin MAR-CP-010
