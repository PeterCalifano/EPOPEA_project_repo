...
T_nodes = fsolve(@(T) HeatBalance(T, R, T_Enc, Q_Sat_T,Q_Sat_L , Q_diss, Q_RTG, sigma_SB),T_guess, options);

T_nodes-273

%%
function [balances] = HeatBalance(T, R, T_E, Q_Sat_T,Q_Sat_L , Q_diss, Q_RTG, sigma)
...
Q_LB = R.R_LB*sigma*(T_L^4-T_B^4);
Q_TB = R.R_TB*sigma*(T_T^4-T_B^4);

% balance lateral surfaces
Q_BL = R.R_BL*sigma*(T_B^4-T_L^4);
Q_TL = R.R_TL*sigma*(T_T^4-T_L^4);
Q_LDS = R.R_LDS*sigma*T_L^4;

% balance top surface
Q_BT = R.R_BT*sigma*(T_B^4-T_T^4);
Q_LT = R.R_LT*sigma*(T_L^4-T_T^4);
