...
%%% DESCRIPTION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Ojective function of the Earth-Saturn interplanetary transfer
%
%%% INPUTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%%% OUTPUTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% DV - Sum of the DSMs and of the DVs given during the Powered Fly Bys
% 
...
for i = 2:(N+1)
    ...
    mu_flyby_planet = astroConstants(20 + planets_id(i));

    % Use 110% of the planet radius as initial guess 
    rp_guess = 1.1 * astroConstants(10 + planets_id(i));

    % Compute the fly-by and the DV
    [DV_i, ~, turning_angle_i] = evaluate_flyby_cost(v_inf_minus, v_inf_plus, mu_flyby_planet, rp_guess);
    DV_fb(i) = DV_i;
end
...
[kep_last,~] = uplanet(times(end), planets_id(end));
[~,V_last] = kep2car(kep_last, mu_S);
mu_end = astroConstants(20 + planets_id(end));
